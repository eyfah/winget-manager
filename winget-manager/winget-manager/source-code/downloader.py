...
def install_or_update_programs(action):
    ...
    if not any([not var.get() for var in check_vars.values()]):
...
for category, apps in categories.items():
    category_label = tk.Label(frame, text=category, font=("Arial", 24, 'bold'), bg=theme["bg"], fg=theme["fg"])
    category_label.grid(row=row, column=0, columnspan=2, sticky='w', padx=5, pady=(10, 5))
    row += 1
    
    for app_name, app_info in apps:
        var = tk.BooleanVar()
        check = tk.Checkbutton(frame, text=app_info.get('content', app_name), variable=var, font=theme["font"], bg=theme["bg"], fg=theme["fg"], selectcolor=theme["highlight_bg"], activebackground=theme["highlight_bg"], activeforeground=theme["fg"], anchor='w', justify=tk.LEFT)
        check_vars[app_name] = var
        check.grid(row=row, column=0, sticky='w', padx=10, pady=5)
        
        # Add question mark button
        question_button = tk.Button(frame, text="[?]", font=theme["font"], bg=theme["button_bg"], fg=theme["button_fg"], command=lambda name=app_name: show_description(name))
...
description_label = tk.Label(right_frame, textvariable=description_text, font=theme["font"], wraplength=300, justify=tk.LEFT, bg=theme["bg"], fg=theme["fg"])
...
social_media_frame = tk.Frame(right_frame, bg=theme["bg"])
social_media_frame.pack(pady=10, padx=10, fill=tk.X)

# GitHub label
github_label = tk.Label(social_media_frame, text="GitHub: eyfah", font=theme["font"], bg=theme["bg"], fg=theme["fg"])
github_label.pack(anchor='w')

# Discord label
discord_label = tk.Label(social_media_frame, text="Discord: qbsy#0000", font=theme["font"], bg=theme["bg"], fg=theme["fg"])
